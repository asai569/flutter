...
const String _beginPlatformAdaptiveGeneratedMark =
    '// BEGIN GENERATED PLATFORM ADAPTIVE ICONS';
const String _endPlatformAdaptiveGeneratedMark =
...
void main(List<String> args) {
  ...
  final String newIconData =
      regenerateIconsFile(iconClassFileData, newTokenPairMap);
...
String regenerateIconsFile(String iconData, Map<String, String> tokenPairMap) {
  final Iterable<_Icon> newIcons = tokenPairMap.entries
  ...
  for (final String line in LineSplitter.split(iconData)) {
    ...
    if (line.contains(_beginPlatformAdaptiveGeneratedMark)) {
      ...
      _platformAdaptiveIdentifiers
          .forEach((String flutterId, List<String> ids) {
        ...
        for (final IconStyle iconStyle in IconStyle.values) {
          ...
          try {
            ...
            platformAdaptiveDeclarations.add(_Icon.platformAdaptiveDeclaration(
                '$flutterId$style', agnosticIcon, iOSIcon));
          } catch (e) {
            if (iconStyle == IconStyle.regular) {
              stderr.writeln(
                  "Error while generating platformAdaptiveDeclarations: Icon '$e' not found.");
              exit(1);
            } else {
    ...
    if (line.contains(_beginGeneratedMark)) {
      ...
      final String iconDeclarationsString =
          newIcons.map((_Icon icon) => icon.fullDeclaration).join('');
...
String _generateDeclaration(MapEntry<String, String> tokenPair) {
  ...
  // to denote style.
  if (webFontKey.endsWith('_outlined') &&
      webFontKey != 'insert_chart_outlined') {
...
class _Icon {
  ...
  _Icon(MapEntry<String, String> tokenPair) {
    ...
    for (final MapEntry<String, String> rewritePair
        in _identifierRewrites.entries) {
      if (id.startsWith(rewritePair.key)) {
        flutterId = id.replaceFirst(
            rewritePair.key, _identifierRewrites[rewritePair.key]);
      }
    }
  }
...
''';
  ...
  static String platformAdaptiveDeclaration(
          String flutterId, _Icon agnosticIcon, _Icon iOSIcon) =>
      '''
