...
const Map<String, String> _kManuallyPinnedDependencies = <String, String>{
  ...
  'mockito':
      '4.1.1', // Prevent mockito from upgrading to the source gen version.
  'vm_service_client':
      '0.2.6+2', // Final version before being marked deprecated.
  'flutter_template_images':
      '1.0.1', // Must always exactly match flutter_tools template.
  'shelf': '0.7.5',
  // nnbd
...
class UpdatePackagesCommand extends FlutterCommand {
  UpdatePackagesCommand() {
    argParser
      ...
      ..addOption(
        'from',
        help:
            'Used with flag --dependency-path. Specifies the package to begin '
            'searching dependency path from.',
      )
      ..addOption(
        'to',
        help:
            'Used with flag --dependency-path. Specifies the package that the '
  ...
  Future<void> _downloadCoverageData() async {
    ...
    final String urlBase =
        globals.platform.environment['FLUTTER_STORAGE_BASE_URL'] ??
            'https://storage.googleapis.com';
    final Uri coverageUri =
        Uri.parse('$urlBase/flutter_infra/flutter/coverage/lcov.info');
  ...
  Future<FlutterCommandResult> runCommand() async {
    ...
    if (upgrade || isPrintPaths || isPrintTransitiveClosure) {
      ...
      final Map<String, PubspecDependency> dependencies =
        ...
        for (final PubspecDependency dependency in pubspec.allDependencies) {
          ...
          if (dependencies.containsKey(dependency.name)) {
            ...
            if (dependency.kind != previous.kind ||
                dependency.lockTarget != previous.lockTarget) {
              throwToolExit(
      ...
      try {
        ...
        fakePackage
            .writeAsStringSync(_generateFakePubspec(dependencies.values));
        ...
        await pub.get(
          ...
          upgrade: true,
          offline: offline,
          flutterRootOverride: upgrade ? temporaryFlutterSdk.path : null,
...
class PubspecYaml {
      ...
      if (lastDependency == null) {
        ...
        if (header != null) {
          ...
          final PubspecDependency dependency =
              PubspecDependency.parse(line, filename: filename);
          if (dependency != null) {
            // We got one!
  ...
  Iterable<PubspecDependency> get allDependencies sync* {
    for (final PubspecLine data in inputData) {
      ...
      if (data is PubspecDependency &&
          transitiveDepsAllowlist.contains(data.name)) {
        yield data;
      }
      if (data is PubspecDependency &&
          data.kind != DependencyKind.overridden &&
...
class PubspecHeader extends PubspecLine {
  PubspecHeader(String line, this.section, {this.name, this.value})
      : super(line);
  ...
  static PubspecHeader parse(String line) {
    ...
    switch (sectionName) {
      ...
      case 'name':
      case 'version':
        return PubspecHeader(line, Section.header,
            name: sectionName, value: value);
      default:
...
class PubspecDependency extends PubspecLine {
  ...
  static PubspecDependency parse(String line, {@required String filename}) {
    ...
    return PubspecDependency(line, package, suffix,
  ...
  bool get pointsToSdk {
    ...
    if (_kind == DependencyKind.path &&
        !globals.fs.path.isWithin(
            globals.fs.path.join(Cache.flutterRoot, 'bin'), _lockTarget) &&
  ...
  void describeForFakePubspec(
      StringBuffer dependencies, StringBuffer overrides) {
    switch (kind) {
  ...
  Iterable<String> getTransitiveDependenciesFor(
    ...
    for (final String dependency in _dependencyTree[package]) {
      if (!seen.contains(dependency)) {
        ...
        seen.add(dependency);
        yield* getTransitiveDependenciesFor(dependency,
            seen: seen, exclude: exclude);
      }
    }
  }
...
String _computeChecksum(
    Iterable<String> names, String getVersion(String name)) {
  int lowerCheck = 0;
  int upperCheck = 0;
