...
class Cache {
  ...
  Cache({
    ...
    if (artifacts == null) {
      ...
      for (final String artifactName in IosUsbArtifacts.artifactNames) {
        _artifacts
            .add(IosUsbArtifacts(artifactName, this, platform: _platform));
      }
  ...
  ArtifactUpdater get _artifactUpdater =>
      __artifactUpdater ??= _createUpdater();
...
class FlutterWebSdk extends CachedArtifact {
  ...
  Future<void> updateInner(
    ...
    final Uri url = Uri.parse(
        '${cache.storageBaseUrl}/flutter_infra/flutter/$version/$platformName.zip');
    if (location.existsSync()) {
...
abstract class EngineCachedArtifact extends CachedArtifact {
  ...
  Future<void> updateInner(
    ...
    final String url =
        '${cache.storageBaseUrl}/flutter_infra/flutter/$version/';
  ...
  Future<bool> checkForArtifacts(String engineVersion) async {
    ...
    final String url =
        '${cache.storageBaseUrl}/flutter_infra/flutter/$engineVersion/';
  ...
  Future<void> update(
    ...
    try {
      ...
      final RunResult processResult = await globals.processUtils.run(<String>[
        gradleExecutable,
        '-b',
        globals.fs.path.join(flutterSdk, 'packages', 'flutter_tools', 'gradle',
  ...
  Future<void> updateInner(
    ...
    fileSystem
        .file(fileSystem.path.join(
            location.path, 'gradle', 'wrapper', 'gradle-wrapper.properties'))
        .deleteSync();
...
class IosUsbArtifacts extends CachedArtifact {
  ...
  Uri get archiveUri => Uri.parse(
      '${cache.storageBaseUrl}/flutter_infra/ios-usb-dependencies${cache.useUnsignedMacBinaries ? '/unsigned' : ''}/$name/$version/$name.zip');
}
  ...
  Future<void> _downloadArchive(
    ...
    while (retries > 0) {
      status = _logger.startProgress(
        message,
        timeout:
            null, // This will take a variable amount of time based on network connectivity.
      );
      try {
