...
void main() {
  ...
  setUp(() {
    ...
    ideviceSyslogPath = artifacts.getArtifactPath(Artifact.idevicesyslog,
  ...
  group('syslog stream', () {
    testWithoutContext('decodeSyslog decodes a syslog-encoded line', () {
      final String decoded =
          decodeSyslog(r'I \M-b\M^]\M-$\M-o\M-8\M^O syslog \M-B\M-/\'
...
''',
  ...
  group('VM service', () {
    testWithoutContext('IOSDeviceLogReader can listen to VM Service logs',
      ...
      final StreamController<Event> stdoutController =
      ...
      when(vmService.streamListen('Stdout'))
          .thenAnswer((Invocation invocation) {
        return stdoutCompleter.future;
      });
      when(vmService.streamListen('Stderr'))
          .thenAnswer((Invocation invocation) {
    ...
    testWithoutContext(
      ...
      when(iosDeployDebugger.logLines)
          .thenAnswer((Invocation invocation) => debuggingLogs);
  ...
  group('debugger stream', () {
    ...
    testWithoutContext('detaches debugger', () async {
      ...
      final MockIOSDeployDebugger iosDeployDebugger = MockIOSDeployDebugger();
      when(iosDeployDebugger.logLines)
          .thenAnswer((Invocation invocation) => const Stream<String>.empty());
      logReader.debuggerStream = iosDeployDebugger;
