...
void main() {
  ...
  group('VMServiceFlutterDriver.connect', () {
    ...
    setUp(() {
      ...
      when(mockIsolate.loadRunnable())
          .thenAnswer((_) => Future<MockIsolate>.value(mockIsolate));
    ...
    test('connects to isolate paused at start', () async {
      ...
      when(mockPeer.sendRequest('streamListen', any))
      ...
      expect(connectionLog,
          <String>['resume', 'streamListen', 'onExtensionAdded']);
    });
  ...
  group('VMServiceFlutterDriver', () {
    ...
    setUp(() {
      ...
      mockIsolate = MockIsolate();
      driver =
          VMServiceFlutterDriver.connectedTo(mockClient, mockPeer, mockIsolate);
    });
  ...
  group('WebFlutterDriver', () {
    ...
    group('WebFlutterDriver Unimplemented/Unsupported error', () {
      test('forceGC', () async {
        expect(driver.forceGC(), throwsA(isA<UnimplementedError>()));
      });

      test('getVmFlags', () async {
        expect(driver.getVmFlags(), throwsA(isA<UnimplementedError>()));
      });
  ...
  group('WebFlutterDriver with non-chrome browser', () {
    ...
    test('tracing', () async {
      ...
      expect(driver.startTracing(), throwsA(isA<UnsupportedError>()));
      expect(driver.stopTracingAndDownloadTimeline(),
          throwsA(isA<UnsupportedError>()));
      expect(driver.clearTimeline(), throwsA(isA<UnsupportedError>()));
    });
  });
}
